...
/**
 * 心跳清理。「自动清理」（保留期设置）与「立即清理」各是一个独立表单，
 * 交互对齐运行记录清理弹窗；心跳没有失败/成功清理范围之分，比运行记录少一个勾选。
 */
export default function MonitorsCleanupDialog({ open, retentionDays, onClose, onRetentionChanged, onCleaned }: Props) {
  ...
  async function saveRetention() {
    ...
    try {
      await put("/api/monitors/retention", { days: retention });
      toast("自动清理保留期已更新");
  ...
  async function cleanNow() {
    ...
    setBusy(true);
    try {
      const d = await post<{ deleted: number }>("/api/monitors/cleanup", { days: parsed });
      toast(`已清理 ${d.deleted} 条心跳`);
      onClose();
      onCleaned();
    } catch (e) {
      toast(errText(e), "err");
  ...
  return (
    <Dialog open={open} onClose={onClose} title="清理心跳记录" width="max-w-sm">
      {/* 表单一：自动清理 */}
      <section>
        ...
        <p className="mt-2 text-xs text-fg-muted">超过保留期的心跳由调度器在每轮探测后自动删除。</p>
