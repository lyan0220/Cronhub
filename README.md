...
- **心跳监控**：对任意 HTTP(S) 地址周期性探测——默认浏览器 UA、可自定义请求头，模拟真实访问；支持期望状态码（单码/多码/区间，默认 200-299）与响应体关键词断言、心跳条带与多窗口在线率展示；离线 / 恢复推送告警，并可**联动触发任务**（如离线自动重启的恢复脚本）
...
所有事件的文案由 `src/server/notify.ts` 的统一构造器生成，只有一套极简版式：**每条通知一行——`[对象名] [emoji 状态] 原因`**，仅联动触发这类罕见信息才另起一行。状态 emoji：🔴 离线、✅ 恢复、❌ 失败、⛔ 自动停用、🔑 PAT 失效、🔔 测试。共七种事件：
...
| `monitor_down` | 监控连续失败达到阈值、判定离线的当轮 | `[{监控名}] [🔴 离线] {错误信息}`，联动时追加 `已联动触发「{任务名}」` |
| `monitor_up` | 离线中的监控探测恢复 | `[{监控名}] [✅ 恢复] HTTP {状态码} · {ms}ms`，联动时追加同上 |
...
- **判定规则**：期望状态码 + 可选的响应体关键词包含检查（仅 GET；读取上限 1MB）。状态码默认 200-299（任意 2xx），支持多码与区间（如 `200,204`、`301-302`）；网络错误、超时、状态码不符、关键词缺失均判失败。
- **防抖与状态机**：连续失败达到「失败阈值」（默认 1 次）才判定离线（down），成功即恢复（up）并清零计数；仅在状态切换的当轮推送 `monitor_down` / `monitor_up` 通知，不随每次失败重复发送。
- **任务联动**：可为每个监控配置「离线时触发」/「恢复时触发」的任务（如自动重启、故障切换的 workflow）。联动在状态切换时触发一次，像手动触发一样记录运行并追踪执行结果；联动触发失败会追加一条 `job_failed` 告警。联动任务不受自身启用状态限制（可以专门建一个只留给联动用的任务）。删除任务后，监控上的联动引用自动置空。
- **暂停时段**：可为每个监控配置每日暂停窗口（如 22:00 – 06:00，支持跨天与自定义时区）；窗口内跳过探测——不产生心跳、不推送告警、不触发联动，窗口结束自动恢复。
- **粒度与保留期**：平台 Cron 每 2 分钟唤醒一次，探测间隔最小 2 分钟；心跳记录默认保留 30 天，可在监控页「清理心跳」弹窗中调整保留期或立即清理（`heartbeats_retention_days`）。手动「立即检测」记录心跳并可立即恢复 up，但不参与离线判定。
