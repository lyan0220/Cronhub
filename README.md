...
- **workflow 结果追踪**：自动轮询 GitHub Runs API 回填 workflow 真实执行结论（成功 / 失败 / 取消等），触发成功 ≠ 跑成功，两个失败维度分开告警与统计
- **失败告警与自动停用**：多通知渠道管理（企业微信 / 飞书 / Telegram / Bark / 通用 JSON），任务级自选推送路径，触发失败与 workflow 真失败均推送，连续失败自动停用
